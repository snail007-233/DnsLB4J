...
public class DnsNodeManager {
	...
	private static final ConcurrentHashMap<String, ConcurrentHashMap> backend = new ConcurrentHashMap<>();
	private static final ConcurrentHashMap<String, ConcurrentHashMap> backup = new ConcurrentHashMap<>();
					...
					try {
						...
						for (int i = 0; i < Cfg.configInt("check_count"); i++) {
							...
							request(Unpooled.copiedBuffer(new Packet().setQuery().id(id).queryDomain(Cfg.config("check_domain")).getBytes()), hostname, Integer.valueOf(port), timeout, (ChannelHandlerContext ctx1, DatagramPacket responsePacket, DatagramPacket requestPacket) -> {
								nodeListMap.get(key).put("status", STATUS_ONLINE);
						...
						Log.logger().debug(nodeListName + " checked " + key);

						if (errorCount.size() >= Cfg.configInt("error_count")) {
							nodeListMap.get(key).put("status", STATUS_OFFLINE);
							Log.logger().warn(nodeListName + " offline " + key);
						} else {
							nodeListMap.get(key).put("status", STATUS_ONLINE);
							Log.logger().debug(nodeListName + " online " + key);
						}
					} catch (Exception e) {
					}
					ThreadUtil.sleep(Cfg.configInt("check_interval"));
